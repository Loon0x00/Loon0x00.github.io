...
export default function MitmCertGenerator() {
  ...
  return <section className={styles.generator} aria-label={en ? 'Generate a MitM certificate locally' : '本地生成 MitM 证书'}>
    ...
    <button type="button" onClick={generate} disabled={busy} className={styles.primary}>
      {busy ? (en ? 'Generating certificate…' : '正在生成证书…') : (en ? 'Generate MitM CA Certificate' : '生成 MitM CA 证书')}
    </button>
    {error && <p role="alert" className={styles.error}>{error}</p>}
    {result && <div className={styles.result}>
      <p>{en ? 'A 2048-bit RSA CA certificate valid for about five years has been generated. Import it into Loon, then install and trust the CA certificate.' : '已生成有效期约 5 年的 RSA 2048 位 CA 证书。请先导入 Loon，再安装并信任 CA 证书。'}</p>
      <div className={styles.actions}>
        ...
        <button type="button" onClick={() => download(result.certificatePem, 'loon-ca.crt', 'application/x-x509-ca-cert')}>{en ? 'Install on iOS Device' : '安装到 iOS 设备'}</button>
      </div>
      <div className={styles.steps}>
        <h2>{en ? 'Use with Loon and iOS' : '在 Loon 和 iOS 设备上使用'}</h2>
        {en ? <ol>
          <li>Generate the certificate in Safari on your iPhone or iPad. Tap “Import into Loon,” open Loon when prompted, and confirm the import. You can also copy the configuration below into the [MitM] section of your Loon profile.</li>
          <li>Return to Safari and tap “Install on iOS Device” to download the certificate, then follow the iOS installation prompts.</li>
          ...
          <li>在 iPhone 或 iPad 的 Safari 中生成证书，点「一键导入 Loon」，按提示打开 Loon 并确认导入；或者复制下面的配置片段，粘贴到 Loon 配置文件的 [Mitm] 部分。</li>
          <li>返回 Safari，点「安装到 iOS 设备」下载描述文件，再到「设置 → 已下载描述文件」完成安装。</li>
          <li>前往「设置 → 通用 → 关于本机 → 证书信任设置」为该根证书开启完全信任，最后在 Loon 中启用 MitM。</li>
        </ol>}
        <p>{en ? <>If the installation prompt does not appear, open <code>loon-ca.crt</code> from Files. The certificate contains only the public CA certificate.</> : <>如果安装提示没有出现，可在“文件”中打开 <code>loon-ca.mobileconfig</code>。描述文件只包含 CA 公钥证书。</>}</p>
      </div>
